...
//declare_id!("3bPns6ALo2HFZiSqah4BQpDfsXMXY1aLSS9S2ALcyjhS");
...
//pub mod basic_oracle_example {
  //  use super::*;
    ...
    impl<'info> ReadOracleData<'info> {
        pub fn read_oracle_data(&mut self) -> Result<()> {
            // Access the oracle data directly
            // The quote_account constraint validates it's the canonical account
            let feeds = &self.quote_account.feeds;

            // Calculate staleness
            let current_slot = self.sysvars.clock.slot;
            let quote_slot = self.quote_account.slot;
