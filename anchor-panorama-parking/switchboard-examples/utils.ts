...
export const MARKETPLACE_PROGRAM_PATH =
  "target/deploy/marketplace-keypair.json";
...
/**
 * Loads the panorama parking marketplace program
 *
 * @param {anchor.Provider} provider - Anchor provider
 * @returns {Promise<anchor.Program>} Basic program instance
 */
export async function loadMarketplaceProgram(
  provider: anchor.Provider
): Promise<anchor.Program> {
  return await myAnchorProgram(provider, MARKETPLACE_PROGRAM_PATH);
}
...
/**
 * Interface for distance sensor data structure
 */
...
/**
 * Builds an oracle job for fetching distance sensor data
 *
 * Creates a job that fetches distance measurements from a sensor API endpoint.
 * The job uses JSONPath to extract the distance value from the response.
 *
 * @param {string} apiUrl - The API endpoint URL that returns sensor data
 * @param {string} sensorName - Optional sensor name filter (e.g., "DS20L")
 * @returns {OracleJob} Oracle job configuration for distance sensor data fetch
 *
 * @example
 * ```typescript
 * // Get distance from first sensor in array
 * const job = buildDistanceSensorJob("https://api.example.com/sensors");
 *
 * // Filter by specific sensor name
 * const job = buildDistanceSensorJob("https://api.example.com/sensors", "DS20L");
 * ```
 */
export function buildDistanceSensorJob(
  apiUrl: string,
  sensorName?: string
): OracleJob {
  // If sensor name is provided, filter by it; otherwise get first element
  const jsonPath = sensorName
    ? `$[?(@.sensor == '${sensorName}')].distance_in_cm`
    : `$[0].distance_in_cm`;
...
/**
 * Parses distance sensor JSON data in TypeScript/JavaScript
 *
 * Helper function to extract distance values from sensor JSON responses.
 * Useful for processing oracle responses or direct API calls.
 *
 * @param {string | object} jsonData - JSON string or parsed object containing sensor data
 * @param {string} sensorName - Optional sensor name filter
 * @returns {number | null} Distance in cm, or null if not found
 *
 * @example
 * ```typescript
 * // Parse from JSON string
 * const json = '[{"id":1,"sensor":"DS20L","distance_in_cm":20}]';
 * const distance = parseDistanceSensorData(json); // Returns: 20
 *
 * // Filter by sensor name
 * const distance = parseDistanceSensorData(json, "DS20L"); // Returns: 20
 *
 * // Parse from API response
 * const response = await fetch("https://api.example.com/sensors");
 * const data = await response.json();
 * const distance = parseDistanceSensorData(data); // Returns: 20
 * ```
 */
export function parseDistanceSensorData(
  jsonData: string | DistanceSensorData[],
  sensorName?: string
): number | null {
  try {
    // Parse if string, otherwise use as-is
    const data: DistanceSensorData[] =
    ...
    const sensor = sensorName
      ? data.find((item) => item.sensor === sensorName)
      : data[0];

    return sensor?.distance_in_cm ?? null;
  } catch (error) {
    console.error("Error parsing distance sensor data:", error);
    return null;
  }
}
